<x-layouts.app title="Music Stats">
    ...
    <x-layout.page-header title="Music Stats">
        <x-slot:actions>
    ...
    <div class="stats-row">
        <x-stats.stat-card label="Total plays" :value="number_format($totalPlays)" />
    ...
    <div class="stats-row" style="margin-top: 1rem;">
        ...
        @if($firstPlay)
            <x-stats.stat-card
                label="First play"
                :value="$firstPlay->played_at->format('d M Y')"
            />
        @endif
        @if($lastPlay)
            <x-stats.stat-card
                label="Last play"
                :value="$lastPlay->played_at->diffForHumans()"
            />
        @endif
    </div>

    <div class="mt-6">
        <x-ui.card title="Plays — last 30 days">
            @if($playsPerDay->isEmpty())
