...
final class ContextBuilderService
{
    ...
    private function introduction(): string
    {
        $lines = [
            ...
            'You also have database query tools to look up data for ANY specific date or time period:',
            '  get_health_data — steps and heart rate for any date',
            '  get_gaming_data — PlayStation gaming sessions for any date',
            '  get_music_data — music listening history for any date',
            '  get_media_data — movies and TV episodes watched for any date',
            '  get_calendar_events — calendar events and appointments for any date',
            '  get_contacts — people with birthdays, ages, and relationship types',
            '  get_trophies — PlayStation trophy progress and earned trophies',
            '  get_work_schedule — work days and hours',
            'Answer in the same language the user writes in (Dutch or English).',
            'Today is '.now()->format('l, j F Y').'.',
            '',
            'TOOL USAGE — MANDATORY:',
            '- You MUST call tools directly and immediately. NEVER ask the user for permission first.',
            '- NEVER say things like "Would you like me to look that up?" or "Shall I check that?" — just call the tool.',
            '- NEVER say you cannot find information before calling a tool to check.',
            '- Whenever a question involves a specific date, a named period (e.g. "last month", "3 weeks ago", "August 2025"), or anything outside the 14-day summary below: call the appropriate tool NOW.',
            '- After calling a tool and receiving results, answer immediately based on those results.',
            '',
            'STRICT RULES — follow these without exception:',
            '1. Only use facts that are explicitly present in the data below or returned by a tool. Never invent, assume or extrapolate details.',
            '2. If specific information is not in the data and a tool returns no results, say exactly that: "I don\'t have that information."',
            '3. Never mention series, games, tracks or episodes that are not listed in the data or tool results.',
